...
Simulacoes= 1:NMC;
plot(Simulacoes,Erro)
xlabel("Nº de runs")
ylabel("Erro relativo")
title("Probabilidade relativa de cada estados em função do número de runs")
str = strings([1,Ncasas]);
for a = 1:Ncasas
str(a) = sprintf("Estado %d", a);
end
legend(str);
figure(1)
plot(Simulacoes,Media_erros)
hold on
plot(Simulacoes,(Media_erros(1)./sqrt(Simulacoes)),'Linewidth',1.5)
xlabel("Nº de runs")
ylabel("Erro relativos")
title("Probabilidade relativa média de todos os estados em função do número de runs")
legend("Média de todos os estados", "Curva de convergência")